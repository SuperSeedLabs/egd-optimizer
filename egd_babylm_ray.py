...
from agent.egd.egd_mgpu import EGD
...
# Removed CUDA_VISIBLE_DEVICES restriction to allow multi-GPU usage
...
def main():
    ...
    # Print Ray and CUDA device information for debugging
    print("Ray is initialized:", ray.is_initialized())
    print("CUDA devices visible to torch:", torch.cuda.device_count())
    ...
    meta_config = {
        ...
        'GENERATIONS': 2,  # Run for 50 generations
    ...
    train_data_percent = 1  # Use 10% of the data for training
    (training_data, validation_data, _) = load_babylm(
        block_size=block_size,
        data_percent=train_data_percent
    )

    # Initialize EGD with distributed_gpus=True to enable multi-GPU training
    egd = EGD(
        ...
        seed=42,
        distributed_gpus=True  # Enable multi-GPU training
    )

    # Debug: Print device IDs of Ray actors to verify distribution
    if hasattr(egd, 'ray_actors') and egd.ray_actors:
        print("\nVerifying Ray actor GPU assignments:")
        for i, actor in enumerate(egd.ray_actors):
            print(f"Actor {i} device ID:", ray.get(actor.device_id.remote()))
